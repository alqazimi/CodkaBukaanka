...
test("public form captcha rejects submissions in production when provider is not configured", async () => {
  ...
  try {
    ...
    if (!result.ok) {
      assert.match(result.error, /security verification/i);
    }
  } finally {
...
test("public form captcha skips verification in development when provider is not configured", async () => {
  const prevUrl = process.env.CAPTCHA_VERIFY_URL;
  const prevSecret = process.env.CAPTCHA_SECRET;
  const prevNodeEnv = process.env.NODE_ENV;
