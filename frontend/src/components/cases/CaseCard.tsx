  ...
  return (
    ...
    <article className="group relative card-interactive flex h-full flex-col overflow-hidden p-5 sm:p-6 before:absolute before:inset-x-0 before:top-0 before:h-0.5 before:scale-x-0 before:bg-gradient-to-r before:from-transparent before:via-red-500 before:to-transparent before:transition-transform before:duration-300 hover:before:scale-x-100">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        {caseItem.caseNumber && <span className="text-xs font-mono text-white/55">{caseItem.caseNumber}</span>}
      ...
      <h3 className="font-serif text-lg font-semibold tracking-tight text-white transition-colors duration-200 group-hover:text-red-100">
      ...
      {description && <p className="mt-2 line-clamp-3 flex-1 text-sm leading-relaxed text-white/75">{description}</p>}

      <div className="relative mt-4 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-white/60">

        {caseItem.hospital?.slug && (

          <Link href={`/hospitals/${caseItem.hospital.slug}`} prefetch className="relative z-10 transition-colors hover:text-white">
        ...
        {patient?.slug && (
          ...
          <>
            ...
            <Link href={`/patients/${patient.slug}`} prefetch className="relative z-10 transition-colors hover:text-white">
        ...
        {caseItem.medication?.slug && (
          ...
          <>
            ...
            <Link href={`/medications/${caseItem.medication.slug}`} prefetch className="relative z-10 transition-colors hover:text-white">
        ...
        {caseItem.incidentDate && <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[11px] font-medium text-white/75 backdrop-blur-sm">{formatDate(caseItem.incidentDate, locale)}</span>}
