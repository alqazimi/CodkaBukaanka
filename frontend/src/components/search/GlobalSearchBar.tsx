...
export function GlobalSearchBar({
  ...
  const iconSize = size === "compact" ? "h-4 w-4 left-3" : "h-5 w-5 left-4";
  const btnSize =
    size === "large"
      ? "px-5 py-2.5 text-sm"
      : size === "compact"
    ...
    <div ref={ref} className={cn("relative", className)}>
      ...
      >
        ...
        <button
          type="submit"
          className={cn(
            "absolute right-2 top-1/2 -translate-y-1/2 rounded-xl bg-gradient-to-r from-teal-600 to-cyan-600 font-semibold uppercase tracking-wide text-white shadow-sm transition hover:from-teal-700 hover:to-cyan-700 min-h-[40px]",
            btnSize
          )}
        >
          {submitLabel}
